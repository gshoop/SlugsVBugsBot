...
uint8_t InitFaceFindingSubHSM(void) {
    ES_Event returnEvent;

    CurrentState = InitPSubState;
    returnEvent = RunFaceFindingSubHSM(INIT_EVENT);
...
ES_Event RunFaceFindingSubHSM(ES_Event ThisEvent) {
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunFaceFindingSubHSM(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        RunFaceFindingSubHSM(ENTRY_EVENT); // <- rename to your own Run function
    }
